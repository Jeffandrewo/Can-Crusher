...
#define BIN_HEIGHT 45    // Bin height in cm (10 inches)
...
#define RELAYMIC1_PIN 22     // Extension relay
#define RELAYMIC2_PIN 21     // Retracti on relay


#define DELAY_TIME 35000  // Time for full extension/retraction in milliseconds
...
void setup() {
  ...
  pinMode(RELAYMIC1_PIN, OUTPUT);
  pinMode(RELAYMIC2_PIN, OUTPUT);
  ...
  digitalWrite(RELAYMIC1_PIN, HIGH);
  digitalWrite(RELAYMIC2_PIN, HIGH);
...
void operateActuator() {
  ...
  if (!irDetected) {
    myServo.write(70); 
    aluminumCanCount++;
    Serial.println("Servo moved to 80 degrees (object detected)");
  } else {
    myServo.write(110);  
    plasticBottleCount++;
    Serial.println("Servo moved to 120 degrees (no object detected)");
  }
  updatePlasticAndAluminumCounts();
  ...
  delay(DELAY_TIME);
  digitalWrite(RELAY1_PIN, HIGH);
  crushCount = plasticBottleCount + aluminumCanCount;
  updateCrushCount();
...
void MicroActuator() {
  Serial.println("Extending RELAYMIC...");
  digitalWrite(RELAYMIC1_PIN, LOW);
  delay(15000);
  digitalWrite(RELAYMIC1_PIN, HIGH);
  Serial.println("RELAYMIC fully extended.");

  Serial.println("Retracting RELAYMIC...");
  digitalWrite(RELAYMIC2_PIN, LOW);
  delay(15000);
  digitalWrite(RELAYMIC2_PIN, HIGH);
  Serial.println("RELAYMIC fully retracted.");
